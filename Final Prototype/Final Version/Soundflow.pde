...
void draw()
{
    ...
    for(int i = j*maxfreq/max_p; i < (j+1)*maxfreq/max_p; i++)
  {
   ...
   stroke(255*fft.getBand(i)/maxband, i*255/maxfreq, 255*(1 - fft.getBand(i)/maxband), 128);
    ...
    //arc(width/(2*max_p)+, height/max_p+j*height/max_p + song.left.get(max_x/8)*height/2, width*i/maxfreq, height*i/maxfreq, PI/2 -2*PI*fft.getBand(i)/maxband, 2*PI*fft.getBand(i)/maxband, OPEN);
    //*
